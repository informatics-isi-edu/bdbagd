...
result files which can then be retrieved via `GET`.  URLs for result files are returned in the response body as 
...
[BDBag](https://github.com/ini-bdds/bdbag) archive file which can then be retrieved via `GET`.  The URL for the result 
bag is returned in the response body as `Content-Type:text/uri-list`.
----
